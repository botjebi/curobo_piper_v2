...
parser.add_argument(
    ...
    default=False,
    help=(
        "Do not zero /World/defaultGroundPlane/ShpereLight. "
        "By default the prim is kept in the stage but its intensity is forced "
        "to 0 so the four randomized workspace SphereLights control lighting."
    ),
)
parser.add_argument(
    ...
    default=3000.0,
    help=(
        "Legacy/fallback fixed intensity for each workspace SphereLight. "
        "Used when --disable_scene_light_randomization is enabled."
    ),
)
parser.add_argument(
    "--scene_light_intensity_min",
    type=float,
    default=50000.0,
    help=(
        "Minimum SphereLight intensity used for per-attempt lighting randomization."
    ),
)
parser.add_argument(
    "--scene_light_intensity_max",
    type=float,
    default=100000.0,
    help=(
        "Maximum SphereLight intensity used for per-attempt lighting randomization."
    ),
)
parser.add_argument(
    "--scene_light_random_seed",
    type=int,
    default=None,
    help=(
        "Optional independent RNG seed for lighting. "
        "Default: derived deterministically from --random_seed so object-pose "
        "sampling and lighting sampling do not affect each other's sequences."
    ),
)
parser.add_argument(
    "--disable_scene_light_randomization",
    action="store_true",
    default=False,
    help=(
        "Keep all four workspace lights at --scene_light_intensity instead of "
        "sampling new independent intensities for each pick-and-place attempt."
    ),
)
parser.add_argument(
    "--scene_light_radius",
...
if args.scene_light_intensity < 0.0:
    parser.error("--scene_light_intensity must be >= 0.")
if args.scene_light_intensity_min < 0.0:
    parser.error("--scene_light_intensity_min must be >= 0.")
if args.scene_light_intensity_max < 0.0:
    parser.error("--scene_light_intensity_max must be >= 0.")
if args.scene_light_intensity_min > args.scene_light_intensity_max:
    parser.error(
        "--scene_light_intensity_min must be <= --scene_light_intensity_max."
    )
...
def disable_default_ground_plane_light(stage):
    """
    Keep the default-ground-plane light prim but force its intensity to zero.

    Expected exact prim path from the current Isaac stage:
        /World/defaultGroundPlane/ShpereLight

    This function MUST be called after add_default_ground_plane(), because that
    helper authors /World/defaultGroundPlane and its child light.
    """
    if args.keep_default_light:
        print(
            "[LIGHT] Ground-plane light left unchanged by flag: "
            "/World/defaultGroundPlane/ShpereLight"
        )
        return []

    expected_path = "/World/defaultGroundPlane/ShpereLight"
    candidate_paths = []

    exact_prim = stage.GetPrimAtPath(expected_path)
    if exact_prim.IsValid():
        candidate_paths.append(expected_path)

    # Robust fallback: if a local Isaac build uses a corrected/different child
    # name, find SphereLight descendants under the default ground-plane prim.
    ground_root = stage.GetPrimAtPath("/World/defaultGroundPlane")
    if ground_root.IsValid():
        try:
            for prim in Usd.PrimRange(ground_root):
                path = str(prim.GetPath())
                if path == "/World/defaultGroundPlane":
                    continue
                try:
                    if prim.IsA(UsdLux.SphereLight) and path not in candidate_paths:
                        candidate_paths.append(path)
                except Exception:
                    pass
        except Exception as exc:
            carb.log_warn(
                f"Failed while scanning defaultGroundPlane light descendants: {exc}"
            )

    if not candidate_paths:
        print(
            "[LIGHT] No SphereLight found under /World/defaultGroundPlane "
            "after ground-plane creation."
        )
        return []
    ...
    for path in candidate_paths:
        try:
            ...
            # Read back the authored value so the terminal confirms the actual
            # stage state instead of only reporting that Set() was called.
            readback = intensity_attr.Get()
            print(
                f"[LIGHT] Disabled ground-plane SphereLight: "
                f"{path} | intensity={readback}"
            )
            disabled.append(path)
        except Exception as exc:
            carb.log_warn(
                f"Failed to zero ground-plane SphereLight {path}: {exc}"
            )
...
def create_workspace_lights(stage):
    ...
    for i, xyz in enumerate(positions, start=1):
        ...
        try:
            # Use the legacy intensity only as the initial stage value. The
            # first pick-and-place attempt will immediately randomize it unless
    ...
    if args.disable_scene_light_randomization:
        mode_text = f'fixed intensity={args.scene_light_intensity:.1f}'
    else:
        mode_text = (
            f'random intensity range=['
            f'{args.scene_light_intensity_min:.1f}, '
            f'{args.scene_light_intensity_max:.1f}]'
        )
...
def set_workspace_light_intensities(stage, light_paths, intensities):
    """Apply one intensity value to each workspace SphereLight."""
    if len(light_paths) != len(intensities):
        raise ValueError(
            f'Light path/intensity length mismatch: '
            f'{len(light_paths)} paths vs {len(intensities)} values'
        )

    applied = []
    for path, intensity in zip(light_paths, intensities):
        light = UsdLux.SphereLight.Get(stage, path)
        prim = stage.GetPrimAtPath(path)
        if not prim.IsValid():
            raise RuntimeError(f'Workspace light prim not found: {path}')

        value = float(intensity)
        if value < 0.0 or not np.isfinite(value):
            raise RuntimeError(
                f'Invalid workspace-light intensity for {path}: {value}'
            )

        attr = light.GetIntensityAttr()
        if not attr:
            attr = light.CreateIntensityAttr()
        attr.Set(value)
...
def randomize_workspace_lights(stage, light_paths, light_rng, attempt_index, reason):
    """
    Sample all four workspace-light intensities independently once per
    pick-and-place attempt, then keep those values fixed for that attempt.
    """
    if args.disable_scene_lights or not light_paths:
        return []

    if args.disable_scene_light_randomization:
        intensities = np.full(
            len(light_paths),
            float(args.scene_light_intensity),
            dtype=np.float64,
        )
        mode = 'fixed'
    else:
        intensities = light_rng.uniform(
            float(args.scene_light_intensity_min),
            float(args.scene_light_intensity_max),
            size=len(light_paths),
        )
        mode = 'random'

    applied = set_workspace_light_intensities(
        stage,
        light_paths,
        intensities,
    )
    ...
    values_text = ', '.join(
        f'L{i}={value:.1f}'
        for i, value in enumerate(applied, start=1)
    )
    print(
        f'[LIGHT] attempt #{attempt_index} ({reason}) | '
        f'mode={mode} | {values_text}'
    )
...
def main():
    ...
    # Keep lighting randomization independent from object-pose randomization.
    # This prevents adding/removing a lighting sample from changing the object
    # spawn sequence (and vice versa).
    light_seed = (
        int(args.scene_light_random_seed)
        if args.scene_light_random_seed is not None
        else (int(args.random_seed) + 100003) % (2**32)
    )
    light_rng = np.random.default_rng(light_seed)
    print(
        f'[LIGHT] RNG seed={light_seed} '
        f'(object random_seed={args.random_seed})'
    )
    ...
    workspace_light_paths = create_workspace_lights(stage)
    ...
    # add_default_ground_plane() authors its own SphereLight, so disable it only
    # after the ground plane exists. Keep the prim itself intact for stability.
    disable_default_ground_plane_light(stage)
    ...
    # Lighting is resampled once for every actual pick-and-place attempt,
    # including retries of the same object pose.
    lighting_attempt_counter = 0
    current_light_intensities = []
    ...
    def begin_new_episode(reason, reset_robot=True, spawn_pose=None, increment_episode=True):
        ...
        nonlocal lighting_attempt_counter, current_light_intensities
        ...
        # Domain-randomize lighting once at the start of every physical
        # pick-and-place attempt. A failure retry uses the same object pose but
        # receives a new independent lighting sample.
        lighting_attempt_counter += 1
        current_light_intensities = randomize_workspace_lights(
            stage,
            workspace_light_paths,
            light_rng,
            attempt_index=lighting_attempt_counter,
            reason=reason,
        )
        ...
        open_task_logger()
        light_text = (
            '[' + ', '.join(f'{v:.1f}' for v in current_light_intensities) + ']'
            if current_light_intensities
            else 'disabled'
        )
        print(
            f"[EPISODE] #{episode_counter} start ({reason}) | "
            f"spawn_object_pos={object_pos}, "
            f"spawn_yaw_deg={math.degrees(object_yaw):.1f}, "
            f"reset_robot={reset_robot}, "
            f"retry_attempt={retry_attempt_counter}, "
            f"light_intensities={light_text}"
        )
